...
function [yn, yorigin] = multi(xqn, x1origin,x2n,x2origin)
    ...
    x1_aligned = zeros(1, length(ny));
    x2_aligned = zeros(1, length(ny));
    
    x1_aligned((min(n1) - n_min) + (1:length(x1n))) = x1n;
    x2_aligned((min(n2) - n_min) + (1:length(x2n))) = x2n;
    
    yn = x1_aligned .* x2_aligned;
